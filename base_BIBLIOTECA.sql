...
-- Tiempo de generación: 26-03-2026 a las 13:57:48
...
--
DELIMITER $$
CREATE TRIGGER `eliminar_autor_aud` AFTER DELETE ON `autor` FOR EACH ROW INSERT INTO audi_socio(audi_accion, audi_fechaModificacion, socNombre_anterior)
VALUES ('Eliminación autor', NOW(), OLD.aut_apellido)
$$
DELIMITER ;
DELIMITER $$
...
--
-- Estructura Stand-in para la vista `reporte_libros`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `reporte_libros` (
...
--
-- Estructura Stand-in para la vista `reporte_prestamos`
-- (Véase abajo para la vista actual)
--
...
--
-- Estructura para la vista `reporte_libros`
--
DROP TABLE IF EXISTS `reporte_libros`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `reporte_libros`  AS SELECT `l`.`lib_titulo` AS `lib_titulo`, `a`.`aut_apellido` AS `aut_apellido` FROM ((`autor` `a` join `tipo_autores` `t` on(`a`.`aut_codigo` = `t`.`copiaAutor`)) join `libro` `l` on(`l`.`lib_isbn` = `t`.`copiaISBN`)) ;

-- --------------------------------------------------------

--
-- Estructura para la vista `reporte_prestamos`
--
DROP TABLE IF EXISTS `reporte_prestamos`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `reporte_prestamos`  AS SELECT `s`.`soc_nombre` AS `soc_nombre`, `p`.`pres_fechaPrestamo` AS `pres_fechaPrestamo` FROM (`socio` `s` join `prestamo` `p` on(`s`.`soc_numero` = `p`.`soc_copiaNumero`)) ;
...
ALTER TABLE `libro`
  ...
  ADD KEY `titulo_libro_idx` (`lib_titulo`);
...
-- Eventos
--
CREATE DEFINER=`root`@`localhost` EVENT `borrar_prestamos` ON SCHEDULE EVERY 1 DAY STARTS '2026-03-26 07:52:32' ON COMPLETION NOT PRESERVE ENABLE DO DELETE FROM prestamo
WHERE pres_fechaPrestamo < DATE_SUB(CURDATE(), INTERVAL 30 DAY)$$
